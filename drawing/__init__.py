...
def c(w, h, canvas):
    return tuple([w, h, draw.canvas(w, h, None)])


@cmdparse.handle('L', int, int, int, int)
def l(x1, y1, x2, y2, canvas):
    w, h, drawing = canvas
    return tuple([w, h, draw.line(x1, y1, x2, y2, drawing)])


@cmdparse.handle('R', int, int, int, int)
def r(x1, y1, x2, y2, canvas):
    w, h, drawing = canvas
    return tuple([w, h, draw.rect(x1, y1, x2, y2, drawing)])


@cmdparse.handle('B', int, int, str)
def b(x1, y1, c, canvas):
    w, h, drawing = canvas
    return tuple([w, h, draw.fill(x1, y1, c, drawing)])
